...
for i, (title, detail, color) in enumerate(steps):
    ...
    if i < len(steps)-1:
        # Arrow spans from right edge of current box to left edge of next box
        ax_flow.annotate('', xy=(x+1.52, 1.2), xytext=(x+0.48, 1.2),
...
fig.suptitle('Water Quality Parameter Distributions — Groundwater vs Surface Water\n'
             'Histograms with KDE, Mean, Median and Standard Deviation  |  Pooled Dataset (2016–2018)',
...
fig.suptitle('Pearson Correlation Coefficient — Groundwater (left) & Surface Water (right)  |  Pooled Dataset (2016–2018)',
...
fig.suptitle('Box Plots of Water Quality Parameters with BIS IS 10500:2012 Threshold Limits\n'
             'Mean (▲) and Median (●) highlighted | Red line = Permissible limit  |  Pooled Dataset (2016–2018)',
...
fig.suptitle('Scatter Plot — K-Means Clustering (k=2) Applied to WQI Feature Space\n'
             'Dimensionality reduced via PCA (PC1 vs PC2)  |  Pooled Dataset (2016–2018)',
...
for ax, X_raw, y_labels, class_names_arr, title, label_col in [
    ...
    sil_sc  = silhouette_score(X_sc, clusters)
    chi_sc  = calinski_harabasz_score(X_sc, clusters)
    ...
    # Cluster quality metrics annotation
    ax.text(0.02, 0.98,
            f'Inertia:      {km2.inertia_:.1f}\n'
            f'Silhouette:   {sil_sc:.3f}\n'
            f'CHI (Calinski-Harabasz): {chi_sc:.1f}',
            transform=ax.transAxes, fontsize=8.5, va='top', fontfamily='monospace',
            bbox=dict(boxstyle='round,pad=0.35', fc='white', ec='#2E86AB', alpha=0.92, lw=1))
...
for metric, fname, ylims, add_perfect in metrics_list:
    ...
    fig.suptitle(f'{metric} — All Classifiers + K-Means  [Test Set Performance]\n'
                 f'Groundwater (left) vs Surface Water (right)  |  Pooled Dataset (2016–2018)',
                 fontsize=14, fontweight='bold', y=1.01, color=PALETTE['primary'])
...
# Short labels for individual subpanels (avoid overlap on small polar axes)
radar_metrics_short = ['GW Acc','GW F1','GW MCC','SW Acc','SW F1','SW MCC','Interp.','CV Stab.']
radar_metrics_mini  = ['GW\nAcc','GW\nF1','GW\nMCC','SW\nAcc','SW\nF1','SW\nMCC','Interp','CV']
...
ax_main.set_xticklabels(radar_metrics_short, size=8.5, fontweight='bold')
ax_main.tick_params(pad=12)
...
for idx, model in enumerate(model_plot_list):
    ...
    ax.set_xticklabels(radar_metrics_mini, size=7.0, fontweight='bold')
...
fig.suptitle('Comprehensive Classifier Performance Dashboard  [Test Set Performance]\n'
             'Groundwater (top row) · Surface Water (bottom row)  |  Pooled Dataset (2016–2018)',
             fontsize=14, fontweight='bold', y=1.01, color=PALETTE['primary'])
